...
parasites_google_clean <- parasites_google_clean |> 
  ...
  ungroup() |> 
  select(a_id, collection_date, age_at_collection_yrs, sn_at_collection, sex, health_status, surrendered, temperament, toxocara_roundworms, cystoisospora_felis_coccidia, sarco_like, toxoplasma, parasite, nemex_dose_number, ponazuril_dose_number, panacur_dose_number) |> 
...
sample_summarize_bar_chart <- function(data, x) { #include data
 ...
 mutate(sn_at_collection = replace_na({{x}}, "Unknown")) |> 
  ...
  geom_bar() +
    scale_fill_brewer(palette = "Greens", na.value = "#dbdbdb") +
  guides(fill = "none") +
  theme_bw() +
...
sample_summarize_bar_chart(parasites_google_clean, age_group) + 
  labs(
...
parasites_google_clean |> 
 ...
 mutate(sn_at_collection = replace_na(sn_at_collection, "Unknown")) |> 
summarize(
    Total = n(),
     Proportion = n()/total
  ) |> 
...
sample_summarize_bar_chart(parasites_google_clean, surrendered) + 
  labs(
...
sample_summarize_bar_chart(parasites_google_clean, temperament) + 
  labs(
...
sample_summarize_bar_chart(parasites_google_clean, health_status) + 
  labs(
...
# graphic
 longer_by_parasites |> 
  ggplot(mapping = aes(x = parasite_type, fill = results)) +
  ...
  theme_bw() +
   ...
   labs(x = "Parasite", 
        ...
        fill = "Result") +
   scale_x_discrete(labels = c("Cystoisospora felis (Coccidia)", "Sarco-like", "Toxocara (Roundworms)", "Toxoplasma"))  # x-axis variable labels
...
#| label: parasite-overview-table
 ...
 #table with proportions
   ...
   longer_by_parasites |> 
    ...
    modify_header(
    label = "**Parasite**",
    stat_1 = "**Negative**, N = {n} ({style_percent(p, digits = 1)}%)",
    stat_2 = "**Positive**, N = {n} ({style_percent(p, digits = 1)}%)",
    stat_0 = "**Overall**, N = {n} ({style_percent(p, digits = 1)}%)" 
  )  |> 
...
```{r}
#| label: roundworm-and-tx-graphic
...
parasites_google_clean |> 
  ...
  scale_fill_brewer(palette = "Greens", na.value = "#dbdbdb",
                    labels = c("Negative", "Positive")) + 
  labs (x = "Number of Nemex Doses Received", y = "Number of Cases", 
...
```{r}
#| label: positive-roundworm-and-tx-by-age-group

parasites_google_clean |> 
  filter(
         toxocara_roundworms == TRUE,
           !is.na(toxocara_roundworms)) |> 
 ggplot(mapping = aes(nemex_dose_number, fill = age_group)) +
  geom_bar(position = "stack") +
theme_bw() +
  scale_fill_brewer(palette = "Greens", na.value = "#dbdbdb",
                    labels = c("Adult", "Juvenile", "Unknown")) + 
  labs (x = "Number of Nemex Doses Received", y = "Positive Roundworms Cases", 
...
parasites_google_clean |>
  select(toxocara_roundworms, nemex_dose_number, age_group) |> 
  tbl_strata(
    strata = toxocara_roundworms,
    .tbl_fun =
      ~ .x |>
        tbl_summary(by = nemex_dose_number, missing = "no") |>
        add_n(),
    .header = "**{strata}**, N = {n}"
  ) |> 
modify_header(
...
parasites_google_clean |> 
  ggplot(mapping = aes(x = ponazuril_dose_number, fill = cystoisospora_felis_coccidia)) +
  geom_bar(position = "stack") +
  #scale_y_continuous(labels = scales::label_percent()) + 
  theme_bw() +
  scale_fill_brewer(palette = "Greens", na.value = "#dbdbdb",
                    labels = c("Negative", "Positive")) + 
  labs (x = "Number of Ponazuril Doses Received", y = "Number of Cats", 
...
parasites_google_clean |> 
 ...
 ggplot(mapping = aes(x = age_group, fill = age_group)) +
  geom_bar() +
theme_bw() +
  scale_x_discrete(drop = FALSE) +
  scale_fill_brewer(palette = "Greens", na.value = "#dbdbdb",
                    labels = c("Adult", "Juvenile", "Kitten")) + 
  labs (x = "Age Group", y = "Positive Coccidia Cases", 
...
parasites_google_clean |>
  select(cystoisospora_felis_coccidia, ponazuril_dose_number, age_group) |> 
  tbl_strata(
    strata = cystoisospora_felis_coccidia,
    .tbl_fun =
      ~ .x |>
        tbl_summary(by = ponazuril_dose_number, missing = "no") |>
        add_n(),
    .header = "**{strata}**, N = {n}"
  ) |> 
modify_header(
